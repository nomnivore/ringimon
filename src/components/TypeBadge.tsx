...
const badgeStyles = cva(
  "flex max-w-fit items-center justify-center rounded-lg border px-3 font-mono",
  {
    variants: {
      type: {
        ...
        Unknown: "bg-zinc-300 border-zinc-500 text-zinc-900",
      },
    },
    defaultVariants: {
      type: "Unknown",
...
export type TypeBadgeProps = VariantProps<typeof badgeStyles>;

const TypeBadge = ({ type = "Unknown" }: TypeBadgeProps) => {
  return <div className={badgeStyles({ type })}>{type || "????"}</div>;
};
